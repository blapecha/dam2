...
function generateJson($result) {
    if ($result && $result->num_rows > 0) {
        ...
        return json_encode(["status" => "success", "data" => $data], JSON_PRETTY_PRINT);
    } else {
        return json_encode(["status" => "error", "message" => "No results found"], JSON_PRETTY_PRINT);
    }
}
...
if ($result && $result->num_rows > 0) {
    ...
    $encodedTableJson = base64_encode($tableJson);
    ...
    echo "<form method='post' action='download.php'>";
    echo "<input type='hidden' name='tablejson' value='" . $encodedTableJson . "'>";
    echo "<button type='submit'>Download as Json</button>";
    echo "</form>";
